...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode):
        ...
        if X_train.shape.index(min(X_train.shape)) != 1:  # 确保通道在第二维度
        ...
        if training_mode == "self_supervised" or training_mode == "SupCon":  # 无需在其他模式下应用增强
...
def data_generator(data_path, configs, training_mode):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, drop_last=configs.drop_last, num_workers=0)
    valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset, batch_size=batch_size, shuffle=False, drop_last=configs.drop_last, num_workers=0)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=0)
